...
from .keymap import map_vk2char, map_char2vk
...
class HotKeyManager:
    ...
    def logger(self, value):
        if value:
            ...
            if 1 == len(self.__logger.handlers):
                self.setLogPath(
                    "{}.log".format(self.__class__.__module__.split(".")[0])
                )
        else:
    ...
    def RegisterHotKey(self, trigger, keys, count=2, interval=0.5, *args, **kwargs):
        ...
        if any([key.keys == keys for key in self.__hot_keys]):
            self.__logger.info(
                "【Register failed】{}: This hot key has been registered".format(keys)
            )
            return -1
        try:
            ...
            e_type = str(type(e))
            self.__logger.info(
                """【Register failed】{}:{}: {}""".format(
                    keys, e_type[e_type.find("'") + 1 : e_type.rfind("'")], e
                )
            )
            return -1
        ...
        if isinstance(key_id, int) and 0 < key_id < self.__id:
            for key in self.__hot_keys:
                if key_id == key.id:
                    ...
                    return True
        self.__logger.info(
            "【Unregister failed】the hot key id: {} doesn't exist".format(key_id)
        )
        return False
    ...
    def __exec_trigger(self, hot_key):
        try:
            ...
            e_type = str(type(e))
            self.__logger.error(
                """【Exception】in {}'s trigger function:{}: {}""".format(
                    hot_key, e_type[e_type.find("'") + 1 : e_type.rfind("'")], e
                )
            )
    ...
    def __on_press(self, key):
        ...
        if isinstance(key, KeyCode):
            key = self.fill_keycode(key=key)
        self.__pressed_keys.append(key)
        self.__logger.debug("【key pressed】{}".format(key))
        for hot_key in self.__hot_keys:
            all_key_pressed = all([key in self.__pressed_keys for key in hot_key.keys])
            if 1 < len(hot_key.keys) and all_key_pressed:
    ...
    def __on_release(self, key):
        ...
        if self.__released_keys:
            for rk in reversed(self.__released_keys):
                interval_ok = hot_key.interval < cur_time - rk.get("time")
                if key == rk.get("key") and interval_ok:
        ...
        if hot_key.count == n:
            self.__released_keys = [
                rk for rk in self.__released_keys if key != rk.get("key")
            ]
            self.__exec_trigger(hot_key)

    def unregister_all(self):
        self.__hot_keys.clear()
        # for key in self.__hot_keys:
        #     self.UnregisterHotKey(key.id)

    def fill_keycode(self, key: KeyCode):
        if not isinstance(key, KeyCode) or (key.vk is None and key.char is None):
            self.__logger.error(f"key {key} is not an instance of KeyCode or key error")
            return key
        if key.char is None:
            key.char = self.vk2char(key.vk)
        if key.vk is None:
            key.vk = self.char2vk(key.char)
        return key

    def vk2char(self, vk: int):
        if vk in map_vk2char:
            return map_vk2char[vk].lower()
        else:
            return None
    
    def char2vk(self, char: str):
        if len(char) != 1:
            self.__logger.error(f"char must be 1 character, get {char}")
            return None
        char = char.upper()
        if char in map_char2vk:
            return map_char2vk[char]
        else:
            return None
